...
DiodePolarityManager::DiodePolarityManager(Database& db)
    : db_(db) {
}

bool DiodePolarityManager::add(const DiodePolarity& polarity, DbResult& res) {
    const char* sql = "INSERT INTO DiodePolarity (Name) VALUES (?);";
    sqlite3_stmt* stmt = nullptr;

    if (!db_.prepare(sql, stmt, res))
        return false;
...
bool DiodePolarityManager::getById(int id, DiodePolarity& polarity, DbResult& res) {
...
bool DiodePolarityManager::list(std::vector<DiodePolarity>& polarities,
    DbResult& res) {
...
bool DiodePolarityManager::remove(int id, DbResult& res) {
